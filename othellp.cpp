...
//    vector<string> s= markMov(map,AISB);
    ...
    int score;
//    score+=320*s.size();
    score+=130*(scoremap[0][0]+scoremap[7][7]+scoremap[0][7]+scoremap[7][0]);
    score-=40*(scoremap[0][1]+scoremap[1][0]+scoremap[6][0]+scoremap[0][6]+scoremap[6][7]+scoremap[7][6]+scoremap[7][1]+scoremap[1][7]);
    for(int i=0;i<8;i++)
        for(int j=0;j<8;j++)
        {
            if(i==0||i==7||j==0||j==7)
